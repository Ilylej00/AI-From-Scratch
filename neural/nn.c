...
NeuralNetwork* network_create(int input, int hidden, int output, double lr){
    NeuralNetwork* ret = malloc(sizeof(NeuralNetwork));

    ret->input = input;
    ret->hidden = hidden;
    ret->output = output;
    ret->learning_rate = lr;
    ret->hidden_weights = matrix_create(hidden, input);
    ret->output_weights = matrix_create(output,hidden);

    // randomize the weights when the network is created 
    matrix_randomize(ret->hidden_weights, hidden);
    matrix_randomize(ret->output_weights, output);
